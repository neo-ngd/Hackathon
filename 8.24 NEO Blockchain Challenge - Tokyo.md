...
　本番環境では各 Transaction Output の状態はよく変わるので、コンペを開始する前に、主催者がNEOのオフライン同期パッケージ（chain.acc.zip）を提供する。このオフライン同期パッケージは、最新高さではないので、各 Transaction Output の状態は、このオフライン同期パケットのブロック高さに基づいている。参加者は、オフライン同期パッケージを使用して、 Transaction Output を独自のデータベースシステムに導入する。（データ導入するには、３時間以内に完成する制限がある）
...
　データ導入時間が3時間以内且つ、すべての支出状況が正しい且つ、照会に使用される時間がより少ない参加者が勝つ。（データ導入時間が3時間に制限される。合計照会時間は20分に制限される。この２つの条件が満たされないチームがほかチームと比べる前に、システムを改善する必要がある）
